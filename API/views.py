...
@csrf_exempt
def temp_api(req):
    with open("temp_api_data.txt", 'w') as file:
        file.write(str(req.POST["data"]))
    ...
    if req.user.is_authenticated:
        if  contest_type in ['ongoing', 'published', 'drafts', 'past']:
            ...
            if req.user.is_superadmin:
                ...
                args = {
                    'contests': Contests.objects.filter(status=contest_type, host = req.user).order_by('-start_date'),
                    'contest_type': contest_type.title(),
                    'is_superadmin': req.user.is_superadmin
                }
...
def draft_contest(req, contest_type, contest_code):
    if req.user.is_authenticated:
        contest = Contests.objects.filter(contest_code=contest_code)
        if contest:
            contest[0].status = 'drafts'
            contest[0].save()
    
    return redirect(f"/codespace/admin_contests/drafts")
...
def start_contest(req, contest_type, contest_code):
    if req.user.is_authenticated:
        ...
        if contest:
            ...
            contest[0].end_date = max(contest[0].end_date, timezone.now() + timedelta(minutes=contest[0].time))
            contest[0].save()
...
def contest_questions_1(req, contest_type, contest_code):
    ...
    if req.user.is_authenticated:
        ...
        if contest:
            ...
            return render(req, 'contest_questions_1.html', args)
...
def add_que(req, contest_type, contest_code):
    ...
    if req.method=="POST":
        ...
        if req.POST['que_type'] == "2":
            ...
            que.debug_language = (req.POST['debug_lang']).lower()
...
def edit_que(req, contest_type, contest_code):
    if req.method == 'POST':
        ...
        if que:
            ...
            que.duration=req.POST['duration']
    ...
    if req.method == 'POST':
        ...
        if que:
            ...
            args = {
                ...
                'debug_lang': que.debug_language,
                'debug_code': que.debug_code,
                'solution': "",
                'inputs': i_o_extract(que.testcases.url) if que.que_type != 1 else "",
                'outputs': i_o_extract(que.outputs.url) if que.que_type != 1 else "",
                'options': que.options.split("@dlim@") if que.options else "",
...
def reset_test_status(req, contest_type, contest_code, user_id):
    if req.user.is_authenticated:
        contest = Contests.objects.filter(contest_code=contest_code)
        if contest:
            contest = contest[0]
            reg = Registrations.objects.filter(contest_id=contest.id, user_id=user_id)
            
            if reg:
                
                reg[0].test_status = 0
                reg[0].que_start_time = None
                reg[0].last_que_no = 0
                reg[0].save()
            
            
            return registrations(req, contest_type, contest_code)
...
def register_contest(req, contest_code):
    if 'username' in req.session:
        ...
        if contest:
            ...
            #     return redirect('/codespace/contests')
            if reg:
                return redirect('/codespace/contests/'+contest_code)
...
def codespace(req, contest_code):
    ...
    if 'username' in req.session:
        ...
        if contest:
            ...
            if contest.time==0:
                rem_time = int((contest.end_date - cur_time).total_seconds())
            else:
                rem_time = int(min(contest.time*60 - (cur_time-start_time).total_seconds(), (contest.end_date - cur_time).total_seconds()))
                
            if rem_time < 1 or (contest.time>0 and rem_time>contest.time*60):
                reg.test_status = 2
                reg.save()
                return redirect(f'/codespace/{ contest_code }/testend')
            ...
            if obj.que_type == 1:
                args = {
                    ...
                    "duration": contest.time, ## if duration = 0, it will be practice with specific ques
                    }
...
def change_que(req, contest_code):
    ...
    if req.method == 'POST':
        ...
        if prev_que.que_type != 1:
            ...
            prev_mcq_selected = True if 'selected_op' in req.POST and req.POST['selected_op'] else False
            if sub and prev_que.que_type == 1:
                ...
                sub[0].status = 'A' if prev_mcq_selected else ' '
                sub[0].save()
            else:
                score = 0
                ...
                Submissions(
                    ...
                    language = req.POST["prev_lang"],
                    score = score,
                    status = 'A' if prev_mcq_selected else ' ',
                    ).save()
...
def edit_mcq(req, contest_type, contest_code):
    if req.method == 'POST':
        ...
        if que:
            ...
            que.score=req.POST['score']
            que.duration = req.POST['duration']
            que.correct_op = req.POST['editcorrect_op']
            que.options = req.POST['options']
